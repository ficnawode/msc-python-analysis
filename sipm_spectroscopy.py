...
h5_file = None
if SAVE_WAVEFORMS:
    h5_file = h5py.File(f"analysis/data/sipm_spectroscopy/signals_{file_timestamp}.h5", "w")

while True:
    time.sleep(0.1)
    new_V_p2p = scope.query_Vp2p(1)
    if len(v_p2ps) == 0 or new_V_p2p != v_p2ps[-1]:
        ...
        df = pd.DataFrame({"t": time_stamps, "V": v_p2ps}).to_csv(f'analysis/data/sipm_spectroscopy/vp2ps_{file_timestamp}.csv')
        
        if SAVE_WAVEFORMS:
            signal = scope.query_Waveform(1)
